...
class App extends React.Component<any, IAppState> {
  ...
  public getSenators(initials: string, results: any) {
    ...
    results.forEach((item: any) => {
      senatorList.push(new Member({id: item.id, name: item.name, party: item.party, role: item.role, twitterId: item.twitter_id}))
    });
          ...
          <div className="SenatorList">
            {this.state.senators.map((senator) => (
              <div key={senator.props.name} className="Senator">
                <Member id={senator.props.id}
                  name={senator.props.name}
